...
if [[ ${RAID_RECOVER} ]]; then
	STATUS="\e[33mWARNING\e[39m - Checked $RAID_DEVICES arrays, recovering : $RAID_RECOVER"
	EXIT=1
elif [[ ${RAID_RESYNC} ]]; then
	STATUS="\e[33mWARNING\e[39m - Checked $RAID_DEVICES arrays, resync : $RAID_RESYNC"
	EXIT=1
...
# All else critical, better save than sorry
	...
	STATUS="\e[33mWARNING\e[39m - Checked $RAID_DEVICES arrays, $RAID_STATUS have failed check: $EXTEND_RAID_STATUS "
	EXIT=1
fi
